...
footer {
  ...
  <body>
    <header class="clearfix">
      ...
      <div id="project">
        <div>Kepada :</div>
        <div>{{$a->nama_pemasok}}</div>
        <div>{{$a->alamat_pemasok}}</div>
        <div>{{$a->kontak_pemasok}}</div>
      </div>
      @endforeach
      <br>
      <br>
      ...
      @foreach($data1 as $b)
        <div id="project">
          <div>Cara Pembelian :{{$b->cara_pembelian}}</div>
          <div>Tanggal Pembelian :{{date_format(date_create("$b->tanggal_pembelian"), "d F Y")}}</div>
          <div>Tanggal Jatuh Tempo :{{date_format(date_create("$b->tanggal_jatuh_tempo"), "d F Y")}}</div>
        </div>
      @endforeach
    </header>
    <main>
      <h4 style="text-align: center;">Detail Pembelian Barang</h4>
      <br>
      <table>
        <thead>
          <tr>
      ...
      <br>
      <h4>Riwayat Riwayat Pembayaran</h4>
      <br>
      <table>
        <thead>
          <tr>
        ...
        <tbody>
          ...
          <tr>
            <td class="service">{{$no++}}</td>
            <td class="desc">{{date_format(date_create("$d->tanggal_pelunasan_hutang"), "d F Y")}}</td>
            <td class="unit">{{ number_format($d->bayar_hutang, 2)}}</td>
          ...
          <tr>
            ...
            <td class="grand total">{{ number_format($d->sisa_hutang, 2)}}</td>
          </tr>
          @endforeach
        </tbody>
      </table>
